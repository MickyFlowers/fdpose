...
from op_test.build import customOps
...
class TripletH5Dataset(PairH5Dataset):
  ...
  def transform_depth_to_xyzmap(self, batch:BatchPoseData, H_ori, W_ori, bound=1):
    ...
    if batch.xyz_mapBs is None:
      ...
      ###
      t0 = time.time()
      src = batch.depthBs.cuda().expand(bs,-1,-1,-1).float().contiguous()
      M = crop_to_oris.cuda().float().contiguous()
      depthBs_ori_ = torch.zeros((bs, 1, H_ori, W_ori), dtype = torch.float32).cuda()
      customOps.korniaGeometryPerspectiveFillZeroNoAlignNearestBCHW(depthBs_ori_, src, M)
      # diff = depthBs_ori.float().contiguous() - depthBs_ori_
      # print("kornia: ", abs(diff[0]).sum(), abs(depthBs_ori.float()[0]).sum())
      depthBs_ori = depthBs_ori_
      t1 = time.time()
      print("score_pre: ", t1-t0)
      ###
      batch.xyz_mapBs = depth2xyzmap_batch(depthBs_ori[:,0], batch.Ks, zfar=np.inf).permute(0,3,1,2)  #(B,3,H,W)
      batch.xyz_mapBs = kornia.geometry.transform.warp_perspective(batch.xyz_mapBs, tf_to_crops, dsize=(H,W), mode='nearest', align_corners=False)
    batch.xyz_mapBs = batch.xyz_mapBs.cuda()
