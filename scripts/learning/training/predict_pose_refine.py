...
from trt_py_test.build import pyTRTEng
...
class PoseRefinePredictor:
  def __init__(self,):
    ...
    self.trt_model_b168 = pyTRTEng.TensorRTEngine(2,2)
    cur_path = os.path.dirname(os.path.realpath(__file__))
    self.trt_model_b168.loadEngineModel(f"{cur_path}/../../trt_py_test/test_refine_f16_b168.eng")
    self.trt_model_b1 = pyTRTEng.TensorRTEngine(2,2)
    self.trt_model_b1.loadEngineModel(f"{cur_path}/../../trt_py_test/test_refine_f16_b1.eng")
  ...
  def predict(self, rgb, depth, K, ob_in_cams, xyz_map, normal_map=None, get_vis=False, mesh=None, mesh_tensors=None, glctx=None, mesh_diameter=None, iteration=5):
    ...
    for _ in range(iteration):
      ...
      for b in range(0, pose_data.rgbAs.shape[0], bs):
        ...
        ###
        t0 = time.time()
        output_trans = torch.zeros((A.shape[0], 3), dtype = torch.float32).cuda()
        output_rot = torch.zeros((A.shape[0], 3), dtype = torch.float32).cuda()
        A = A.contiguous()
        B = B.contiguous()
        if (A.shape[0] == 168):
          self.trt_model_b168.inferEngineModel([A, B], [output_trans, output_rot])
        elif (A.shape[0] == 1):
          self.trt_model_b1.inferEngineModel([A, B], [output_trans, output_rot])
        else:
          raise ValueError("unsupported batch size")
        # diff = output["trans"].float() - output_trans
        # print("pose: ", abs(diff).sum(), abs(output["trans"].float()).sum())
        output = {"trans" : output_trans, "rot" : output_rot}
        t1 = time.time()
        print("pose: ", t1-t0)
        ###
