...
class ScorePredictor:
  def __init__(self, amp=True):
    ...
    print(ckpt_dir)
    ckpt = torch.load(f"{cur_path}/../../weights/2024-01-11-20-02-45/model_best.pth")
    if 'model' in ckpt:
      ckpt = ckpt['model']
    ...
    self.trt_model = pyTRTEng.TensorRTEngine(2,1)
    self.trt_model.loadEngineModel(f"{cur_path}/../../trt_py_test/test_score_f16_b168.eng")
  ...
  def predict(self, rgb, depth, K, ob_in_cams, normal_map=None, get_vis=False, mesh=None, mesh_tensors=None, glctx=None, mesh_diameter=None):
    ...
    def find_best_among_pairs(pose_data:BatchPoseData):
      ...
      for b in range(0, pose_data.rgbAs.shape[0], bs):
        ...
        A = A.contiguous()
        B = B.contiguous()
        output_ = torch.zeros((1, A.shape[0]), dtype = torch.float32).cuda()
        self.trt_model.inferEngineModel([A, B], [output_])
        # diff = output["score_logit"].float() - output_
        # print("score: ", abs(diff).sum(), abs(output["score_logit"].float()).sum())
        output = {"score_logit" : output_}
        t1 = time.time()
        print("score: ", t1 - t0)
        ###
